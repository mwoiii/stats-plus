...
namespace StatsMod {
    public class PlayerStatsDatabase {
        ...
        // "critHeal", "multiKillCount", "level", "acceleration", "experience", "isPlayerControlled", "isSprinting", "outOfDanger", "jumpPower", "maxJumpHeight", "shouldAim", "bestFitRadius", "spreadBloomAngle", "corePosition", "footPosition", "radius", "aimOrigin", "isBoss", "isElite"
        ...
        public string GetStatSeriesAsString(string name, bool rVector = false) // For logging porpoises
        {
            ...
            foreach (object entry in series) {
                if (!rVector) { a.Append($"{entry}, "); } else { a.Append($"{Numberise(entry)}, "); }
            }
        ...
        public static object Numberise(object value) // Takes non numerical object types that can represent numbers and turns them into numerical types to be interpreted.
        {
            ...
            {
                try { return Int64.Parse(valueString, System.Globalization.NumberStyles.AllowThousands); } catch (OverflowException) { return -1; }  // Safeguarding, to prevent data from not being recorded at all
