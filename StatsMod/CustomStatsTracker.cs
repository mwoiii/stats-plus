...
namespace StatsMod
{
    public static class CustomStatsTracker
    {
        ...
        private static Dictionary<PlayerCharacterMasterController, uint> likelyDonations = [];   // The total number of times a player presumably bought an item that was taken by another player
        ...
        public static object GetStat(PlayerCharacterMasterController player, string statName)
        {
            try
            {
                switch (statName)
                {
                    ...
                    case "likelyDonations":
                        return likelyDonations[player];
        ...
        private static void DonationsTrack(On.RoR2.GenericPickupController.orig_AttemptGrant orig, GenericPickupController self, CharacterBody body)
        {
            if (itemPurchaser.ContainsKey(self))
            {
                ...
                {
                    if (likelyDonations.ContainsKey(purchaser)) { likelyDonations[purchaser] += 1; }
                    else { likelyDonations.Add(purchaser, 1); }
                }
